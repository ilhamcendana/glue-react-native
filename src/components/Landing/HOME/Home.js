...
export default Home = ({ navigation }) => {
    ...
    const fetchPost = () => {
        setLoading(true);
        fetch('https://us-central1-forumpengaduan.cloudfunctions.net/getAllPosts')
            .then(res => res.json())
            .then(resjson => setPost(prev => {
                let data = { ...prev };
                data.allPost = resjson;
                return data;
            }))
            .then(() => {
                setRefreshing(false);
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => (
                    <View style={HOMESTYLES.card}>
                        ...
                        <View style={{ paddingHorizontal: 10 }}>
                            <Button title='Lihat Post' raised onPress={() => navigate('OpenedPost', { postIndex: post.allPost[index] })} />
                        </View>
                    </View>
                )}
            />
